...
def load_data():
    ...
    # items
    items = pd.read_csv("../../../data/favgrocery/items.csv").set_index("item_nbr")
    stores = pd.read_csv("../../../data/favgrocery/stores.csv").set_index("store_nbr")
...
def load_unstack(filename):
    df_name, promo_name = 'df_' + filename + '_raw', 'promo_' + filename + '_raw'
    df_2017 = feather.read_dataframe(df_name).set_index(['store_nbr','item_nbr'])
    df_2017.columns = pd.to_datetime(df_2017.columns)
    promo_2017 = feather.read_dataframe(promo_name).set_index(['store_nbr','item_nbr'])
